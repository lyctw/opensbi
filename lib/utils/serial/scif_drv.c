...
#define SCIF_FCR_RST_ASSRT_TFRF \
	(SCIF_RFRST | SCIF_TFRST)	  /* Reset assert tx-FIFO & rx-FIFO   */
...
#define SCIF_SCR_RCV_TRN_EN \
	(SCIF_RE | SCIF_TE) /* Enable receive & transmit w/SC_CLK=no output */
...
#define SCIF_SIZE(offset)                                                   \
	((offset == SCIF_BRR_0_OFFSET) || (offset == SCIF_FTDR_0_OFFSET) || \
	 (offset == SCIF_FRDR_0_OFFSET) || (offset == SCIF_SEMR_0_OFFSET))
...
int scif_init(unsigned long base, unsigned long clk, unsigned long baudrate)
{
	...
	set_reg(SCIF_SCR_0_OFFSET,
		SCIF_SCR_RCV_TRN_DIS); /* Disable receive & transmit */
	set_reg(SCIF_FCR_0_OFFSET,
		SCIF_FCR_RST_ASSRT_TFRF); /* Reset assert tx-FIFO & rx-FIFO */
	...
	set_reg(SCIF_SCR_0_OFFSET,
		0x0000U); /* Select internal clock, SC_CLK pin unused for output pin */

	set_reg(SCIF_SMR_0_OFFSET,
		0x0000U); /* Set asynchronous, 8bit data, no-parity, 1 stop and Po/1 */

	data16 = get_reg(SCIF_SEMR_0_OFFSET);
	set_reg(SCIF_SEMR_0_OFFSET,
		data16 & (~SCIF_SEMR_MDDRS)); /* Select to access BRR */
	...
	set_reg(SCIF_FCR_0_OFFSET,
		SCIF_FCR_RST_NGATE_TFRF); /* Reset negate tx-FIFO, rx-FIFO. */

	set_reg(SCIF_SCR_0_OFFSET,
		SCIF_SCR_RCV_TRN_EN); /* Enable receive & transmit w/SC_CLK=no output */
